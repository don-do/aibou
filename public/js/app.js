...
//
//
 // VuexのmapState関数、mapGetters関数

/* harmony default export */ __webpack_exports__["default"] = ({
  computed: _objectSpread(_objectSpread({}, Object(vuex__WEBPACK_IMPORTED_MODULE_1__["mapState"])({
    // VuexのmapState関数。当算出プロパティとstore/auth.jsのステートを、ひも付け
...
/* harmony default export */ __webpack_exports__["default"] = ({
  // message.jsファイルのcontentの更新を参照。値があればメッセージを表示
  // PhotoForm.vueにて投稿完了後、メッセージ登録時にcontentは更新される
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    onFileChange: function onFileChange(event) {
      ...
      var reader = new FileReader(); // 画像ファイルを読み込み終わったタイミングで実行する処理
          ...
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                // ローディングを表示
                _this2.loading = true;
                formData = new FormData(); // FormData APIを使用。Ajaxでファイルを送るため

                formData.append('photo', _this2.photo); // LaravelでAPIを作成する場合、routes/api.phpにルートを記載するが、
        ...
        return _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default.a.wrap(function _callee$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              ...
              case 2:
                if (_this.apiStatus) {
                  // apiStatusがfalseなら、トップページへ移動しない
                  // トップページに移動。this にルーターオブジェクトを表す $router が追加されている。router.jsにて、 Vue.use(VueRouter) と記述したため
                  _this.$router.push('/');
                }
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
        ...
        return _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default.a.wrap(function _callee2$(_context2) {
          while (1) {
            switch (_context2.prev = _context2.next) {
              case 0:
                _context2.next = 2;
                return axios.put("/api/photos/".concat(id, "/praise"));
    ...
    praiseless: function praiseless(id) {
      var _this3 = this;
        ...
        return _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default.a.wrap(function _callee3$(_context3) {
          while (1) {
            switch (_context3.prev = _context3.next) {
              case 0:
                _context3.next = 2;
                return axios["delete"]("/api/photos/".concat(id, "/praise"));
...
window.axios = __webpack_require__(/*! axios */ "./node_modules/axios/index.js"); // Axiosライブラリの設定を記述
// Ajaxリクエストであることを示すヘッダー（X-Requested-With）を付与

window.axios.defaults.headers.common['X-Requested-With'] = 'XMLHttpRequest'; // フォームではなく、ヘッダーを見てCSRFトークンチェックを行うようにする
...
var routes = [{
  ...
  // PhotoListコンポーネントに、クエリパラメータpageの値をpropsとして渡す
  props: function props(route) {
    // 引数にルート情報routeが入る
    var page = route.query.page; // ルート情報routeから、クエリパラメータのpageの値を取り出す

    return {
      page: /^[1-9][0-9]*$/.test(page) ? page * 1 : 1
    }; // 自然数ならtrue。falseの場合、「1」
  }
}, {
  // 写真詳細ページのルートを定義
  path: '/photos/:id',
  // :id にて、パラメータとして定義。写真IDが入る
  component: _pages_PhotoDetail_vue__WEBPACK_IMPORTED_MODULE_3__["default"],
  // :id の値を、<PhotoDetail> コンポーネントにpropsとして渡す
...
var mutations = {
  setUser: function setUser(state, user) {
    // userステートの値を更新。第一引数state、第二引数に実引数が渡される
    state.user = user;
  },
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              ...
              context.commit('setApiStatus', null); // 会員登録API呼び出し
            ...
            case 3:
              ...
              // 成功した場合
              context.commit('setApiStatus', true); // setUserミューテーションを実行し、userステートを更新
      ...
      return _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default.a.wrap(function _callee2$(_context2) {
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              // apiStatusを通信結果によって更新。最初はunllを入れておく
              context.commit('setApiStatus', null); // ログインAPI呼び出し
      ...
      return _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default.a.wrap(function _callee3$(_context3) {
        while (1) {
          switch (_context3.prev = _context3.next) {
            case 0:
              // apiStatusを通信結果によって更新。最初はunllを入れておく
              context.commit('setApiStatus', null); // ログアウトAPI呼び出し
      ...
      return _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default.a.wrap(function _callee4$(_context4) {
        while (1) {
          switch (_context4.prev = _context4.next) {
            case 0:
              // apiStatusを通信結果によって更新。最初はunllを入れておく
              context.commit('setApiStatus', null); // ユーザーAPI呼び出し
...
/* harmony default export */ __webpack_exports__["default"] = ({
  namespaced: true,
  // ステート名やミューテーション名が同じでも、モジュール名で区別するため
  state: state,
  getters: getters,
  mutations: mutations,
  actions: actions
...
/* harmony import */ var _message__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./message */ "./resources/js/store/message.js");
 ...
 // サクセスメッセージ用のモジュール読み込み
// Vuex プラグインの使用を宣言

vue__WEBPACK_IMPORTED_MODULE_0___default.a.use(vuex__WEBPACK_IMPORTED_MODULE_1__["default"]);
var store = new vuex__WEBPACK_IMPORTED_MODULE_1__["default"].Store({
  modules: {
    // ストア作成時、各ファイルをモジュールとして登録。可読性のため
...
function getCookieValue(searchKey) {
  ...
  var val = ''; // document.cookieで、クッキーをname=12345;token=67890;key=abcdeの形式で参照

  document.cookie.split(';').forEach(function (cookie) {
    // splitで、["name=12345", "token=67890", "key=abcde"]となる
    var _cookie$split = cookie.split('='),
        _cookie$split2 = _slicedToArray(_cookie$split, 2),
        key = _cookie$split2[0],
        value = _cookie$split2[1]; // keyとvalueに切り分け
